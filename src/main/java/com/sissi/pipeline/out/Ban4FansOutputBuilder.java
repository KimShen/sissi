 ...
 */
public class Ban4FansOutputBuilder implements OutputBuilder {
	...
	private class BanOutput implements Output {
		...
		public Boolean output(JIDContext context, Element node) {
			JID mayBan = Ban4FansOutputBuilder.this.jidBuilder.build(node.getFrom());
			if (this.isEmpty(context, mayBan)) {
				return true;
			}
			return !(this.isMatchNode(node) ? Ban4FansOutputBuilder.this.context.isBan(context.getJid(), mayBan) : false);
		}
