...
public class DiscoInfoProcessor extends ProxyProcessor {
	...
	public boolean input(JIDContext context, Protocol protocol) {
		...
		protocol.cast(DiscoInfo.class).setData(this.vcardContext.get(group, new XData().setType(XDataType.RESULT).add(this.form))).add(this.identity.clone().setName(group.user())).add(this.feature);
