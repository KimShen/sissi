...
public class MessageMuc2InviteProcessor extends ProxyProcessor {
	...
	public boolean input(JIDContext context, Protocol protocol) {
		Invite invite = protocol.cast(Message.class).getUser().password(this.mucConfigBuilder.build(super.build(protocol.parent().getTo())).pull(MongoConfig.FIELD_PASSWORD, String.class)).getInvite();
