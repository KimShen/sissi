...
abstract class PresenceRoster2MasterItemProcessor extends ProxyProcessor {
	...
	protected Protocol prepare(JID master, JID slave, RosterSubscription subscription) {
		...
		return new IQ().add(new Roster(new GroupItem(slave.asStringWithBare(), relation.getName(), subscription.toString(), RelationRoster.class.cast(relation).asGroup()))).setTo(master).setType(ProtocolType.SET);
	}
}
