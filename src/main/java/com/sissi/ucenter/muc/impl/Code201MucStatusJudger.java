...
public class Code201MucStatusJudger implements MucStatusJudger {
	...
	public MucStatus judege(MucStatus status) {
		return status.loop() && this.config.collection().findOne(BasicDBObjectBuilder.start(this.query.toMap()).add(MongoConfig.FIELD_JID, status.group()).get()) != null ? status.add("201") : status;
	}
}
