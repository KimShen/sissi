...
public class MatchAllTest {
	...
	public void testMatchAll() {
		...
		NothingProcessor nothing = new NothingProcessor();
		Condition processor3 = new Condition(new AllMatcher(), nothing);
		ChainedFinder finder = new ChainedFinder(Lists.asList(processor1, new Condition[] { processor2, processor3 }));
		Assert.assertSame(stream, finder.find(new Stream()));
		Assert.assertSame(bind, finder.find(new Bind()));
		Assert.assertSame(nothing, finder.find(new IQ()));
	}
}
